...
- `XXXperience.zip` ya incluye una experiencia jugable base (Neon Hunt). `STsLK3R_Z0NE` ya no muestra estados "próximamente" en tabs sociales, pero mantiene fallback/mock para parte de sus datos (`components/apps/XXXperience/XXXperience.tsx`, `components/apps/StalkerZone/StalkerZone.tsx`).
